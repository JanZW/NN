...
def init_neurons(count):
    ...
    return [[random.uniform(0, 1.0),
             random.uniform(0, 1.0)] for i in range(count)]
...
def normalize(cities):
    ...
    min_x = min(cities,key=itemgetter(0))[0]
    max_y = max(cities,key=itemgetter(1))[1]
    min_y = min(cities,key=itemgetter(1))[1]
    s_x=(max_x-min_x)/2
    s_y=(max_y-min_y)/2
    m = max(max(max_x,-min_x), max(-min_y,max_y))/2

    return m, [((x-min_x)/(max_x-min_x), (y-min_y)/(max_y-min_y)) for (x, y) in cities]
